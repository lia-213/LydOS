...
- [x] **Proper header parsing** ⭐⭐ — case-insensitivity done via a hand-rolled `HeaderDict(abc.MutableMapping)`; duplicate headers and folded headers still open (see below).
  ...
  - [ ] Duplicate headers — `header_dict[k] = v` currently overwrites on repeat keys; last one wins, earlier values silently dropped.
  - [ ] Folded headers — a continuation line (starts with whitespace, no colon) currently hits the "no colon → this must be the request line" branch, corrupting the parsed request path.
