...
Assert-InIndex    "sub${sep}nested.txt"      $index "Nested file staged"
Assert-InIndex    "sub${sep}deeper${sep}deep.txt" $index "Deeply nested file staged"
Assert-NotInIndex ".venv${sep}fake_venv_file.txt" $index "Ignored (.venv) file NOT staged"
Assert-NotInIndex ".ugit${sep}index"        $index ".ugit internals NOT staged"
...
if ((Test-Path root1.txt) -and (Test-Path root2.txt) -and (Test-Path sub/nested.txt) -and (Test-Path sub/deeper/deep.txt)) {
